...
export class EventCardComponent {
  ...
  shareQrCodeByEmail(event: EventDto): void {
    ...
    const body = encodeURIComponent(`Here is the QR code for event ${event.name} (ID: ${event.id}).`);
    window.open(`mailto:?subject=${subject}&body=${body}`, '_blank');
  }

  shareQrCodeByWhatsapp(event: EventDto): void {
    const message = encodeURIComponent(`Check out the QR code for event ${event.name} (ID: ${event.id}).`);
